...
local function lock_group_namebot(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_namebot_lock = data[tostring(target)]['settings']['namebot']
  if group_namebot_lock == 'yes' then
    return 'namebot is already locked'
  else
    data[tostring(target)]['settings']['namebot'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'namebot has been locked'
  end
end

local function unlock_group_namebot(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_namebot_lock = data[tostring(target)]['settings']['namebot']
  if group_namebot_lock == 'no' then
    return 'namebot is not locked'
  else
    data[tostring(target)]['settings']['namebot'] = 'no'
    save_data(_config.moderation.data, data)
    return 'namebot has been unlocked'
  end
end
...
local function lock_group_badword(msg, data, target)
  ...
  local group_badword_lock = data[tostring(target)]['settings']['badword']
  if group_badword_lock == 'yes' then
    return 'badword is already locked'
  else
    data[tostring(target)]['settings']['badword'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_badword(msg, data, target)
  ...
  if group_badword_lock == 'no' then
    ...
    data[tostring(target)]['settings']['badword'] = 'no'
    save_data(_config.moderation.data, data)
...
local function lock_group_alllink(msg, data, target)
  if not is_momod(msg) then
    return 
  end
  local group_alllink_lock = data[tostring(target)]['settings']['alllink']
  if group_alllink_lock == 'yes' then
    return 'alllink protection is already enabled'
  else
    data[tostring(target)]['settings']['alllink'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'alllink protection has been enabled'
  end
end

local function unlock_group_alllink(msg, data, target)
  if not is_momod(msg) then
    return 
  end
  local group_alllink_lock = data[tostring(target)]['settings']['alllink']
  if group_alllink_lock == 'no' then
    return 'alllink protection is already disabled'
  else
    data[tostring(target)]['settings']['alllink'] = 'no'
    save_data(_config.moderation.data, data)
    return 'alllink protection has been disabled'
  end
end
...
local function lock_group_sing(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_sing_lock = data[tostring(target)]['settings']['sing']
  if group_sing_lock == 'yes' then
    return 'sing is already locked'
  else
    data[tostring(target)]['settings']['sing'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'sing has been locked'
  end
end

local function unlock_group_sing(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_sing_lock = data[tostring(target)]['settings']['sing']
  if group_sing_lock == 'no' then
    return 'sing is not locked'
  else
    data[tostring(target)]['settings']['sing'] = 'no'
    save_data(_config.moderation.data, data)
    return 'sing has been unlocked'
  end
end
...
function show_supergroup_settingsmod(msg, target)
	...
	if data[tostring(target)]['settings'] then
	  ...
	  if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['badword'] then
			data[tostring(target)]['settings']['badword'] = 'no'
		end
	end
	  if data[tostring(target)]['settings'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['sing'] then
			data[tostring(target)]['settings']['sing'] = 'no'
		end
		end
	if data[tostring(target)]['settings'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['namebot'] then
			data[tostring(target)]['settings']['namebot'] = 'no'
		end
	end
	if data[tostring(target)]['settings'] then
  ...
  if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['alllink'] then
			data[tostring(target)]['settings']['alllink'] = 'no'
		end
		end
	if data[tostring(target)]['settings'] then
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		lock_group_namebot(msg, data, target),
		lock_group_number(msg, data, target),
		lock_group_alllink(msg, data, target),
		lock_group_sing(msg, data, target),
      	}
      	return lock_group_all(msg, data, target), safemode
			...
			if matches[2] == 'alllink' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked link posting ")
				return lock_group_alllink(msg, data, target)
			end
			if matches[2] == 'sing' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked sing")
				return lock_group_sing(msg, data, target)
			end
			...
			if matches[2] == 'namebot' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked namebot")
				return lock_group_namebot(msg, data, target)
			end
		end
		...
		unlock_group_namebot(msg, data, target),
		unlock_group_number(msg, data, target),
		unlock_group_sing(msg, data, target),
		unlock_group_alllink(msg, data, target),
      	}
      	return unlock_group_all(msg, data, target), dsafemode
			...
			if matches[2] == 'sing' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked sing")
				return unlock_group_sing(msg, data, target)
			end
			if matches[2] == 'alllink' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] تبلیغ غیرفعال")
				return unlock_group_alllink(msg, data, target)
			end
			if matches[2] == 'join' then
			...
			if matches[2] == 'namebot' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked namebot")
				return unlock_group_namebot(msg, data, target)
			end
		end
if matches[2] == 'satanoff' then
return 'این ستینگو satanoff درست کرده'
end
